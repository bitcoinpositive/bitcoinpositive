...
#if defined(NDEBUG)
# error "BitcoinPositive cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf9c54701c8c1fe3c5817fc2c00de0baf88a0157c78b55481291d279d3e0e0e9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinPositive: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinPositive Signed Message:\n";
...
{
    // BitcoinPositive: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinPositive: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinPositive: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1513303220, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Beat the year 2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 400 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c168d1f1e2252d411dde5ddba35a6a3fb5c95153836c039811bf33c6ef2c2c3e5a4679c2836a1a323477da579bf466fc8d7bedf1f0144bef31988cbe99c08c33") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513303220;
        ...
        assert(block.hashMerkleRoot == uint256("0x1f76f3919a084cc07c824aaa1942f525409b5a5a48104ebf3cb18ee854f2ab13"));
...
unsigned char pchMessageStart[4] = { 0x2f, 0x7b, 0x15, 0x9d }; // BitcoinPositive: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinPositiveMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinPositiveMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinPositiveMiner(CWallet *pwallet)
{
    printf("BitcoinPositiveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinpositive-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinPositiveMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinPositiveMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinPositiveMiner, pwallet));
}
